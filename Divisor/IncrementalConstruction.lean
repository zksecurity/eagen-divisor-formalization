/-
  ...
  building-block layer for that construction, plus the fully proved
  explicit length-4 base case:

  * **Chord-line core.** `chordCoordRingElt P Q` with three-branch
    dispatch (chord / tangent at non-2-torsion / vertical);
    non-zero-ness and evaluation lemmas at `P`, `Q`, and the third
    intersection; `normPoly` factorisations and degrees per branch;
    exact per-point `ordAt` multiplicities.
  * **Products and division.** `mulCoordRingElt` (commutativity,
    evaluation, `normPoly_mul_eq`), `iterDivLin`, and
...
Combining the per-branch `splitsOnE_chordCoordRingElt_*` lemmas with
`sum_ordAt_eq_natDegree_under_split` gives the total ordAt mass on
`E.points` exactly: 3 for non-vertical chord/tangent lines. -/
