/-
  ...
  * the `β`-weighted group sum on `E.points` vanishes.
  ...
  modulo the degree-sum being an inequality instead of an equality.
-/
import Divisor.Defs
...
/-- **Principal divisor of a nonzero `D ∈ F_q[E]`**
    ...
    * the `β`-weighted group sum on `E.points` is `0`
    ...
    Previously an axiom with `=` in the third clause; the equality
    direction was falsified by Aristotle's counterexample
    (see `docs/divisor-degree-axiom-bug.md`) and is now weakened to
    `≤` — the unconditional bound `betaConstructive_sum_le_degE` that
    suffices for the paper's soundness argument (Step 6 of the
    `step:lift` proof uses only `n_R ≤ degE(D) ≤ degBound < q`). -/
