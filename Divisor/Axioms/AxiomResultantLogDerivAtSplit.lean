/-
  ...
  This is the citable narrowing of the chord-fiber log-derivative
  identity. The chord-specific theorem
...
`Tr_{L/K}(dα/α) = d N_{L/K}(α)/N_{L/K}(α)` (Lang VI.5 Theorem 5.1)
specialised to the polynomial / resultant setting via the
implicit-function differentiation of Lang VIII.5 Theorem 5.1 Case 1.
...
2. Extend the formal `T`-derivation on `K[T]` to `L` via
   ...
   formula `ξ′ = -f^D(ξ)/f'(ξ)`).
3. In `L`, we have `F(T) = c · ∏ᵢ g(ξᵢ, T)` (a finite product); apply
   `Differential.logDeriv_multisetProd` to take logs.
