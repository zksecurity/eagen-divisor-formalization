...
/-- Backward-compatible name for the point-count MA completeness theorem. -/
theorem ma_completeness_clean
  ...
  ma_completeness E stmt wit hk hValid msg hkm hDeg hDegK hAdm hHonestDivisor hD

/-- **MA completeness, field-size form.** Trivial conversion of
    `ma_completeness` via the fiber bound `|E.points| ≤ 2q` for
    `q ≥ 5` (`points_card_le_two_q`; no axiom): the rejection-set
...
/-- **IP completeness, field-size form.** Trivial conversion of
    `ip_completeness` via the fiber bound `|E.points| ≤ 2q` for
    `q ≥ 5` (`points_card_le_two_q`; no axiom): the rejection-set
