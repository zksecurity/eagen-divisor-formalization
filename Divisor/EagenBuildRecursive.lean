...
theorem accInv_level0_chord_divisor_identity_at_infinity
  ...
  have h_run_ne_zero : (ECPoint.affineOfMem E h_acc : ECPoint E) ≠ 0 := by
    -- affineOfMem unfolds to .some _ _ hns; .some ≠ 0.
    intro h_eq
...
theorem accInv_level0_chord_divisor_identity_at_off_support
  ...
  have h_ne_neg : (ECPoint.affine E x y : ECPoint E)
    ...
    apply h_off_A2
    -- h : .some _ _ hns = .some _ _ hns_third → (x, y) = (Q_0x, Q_0y).
    have : x = Q₀x ∧ y = Q₀y := by
...
For R = .some _ _ hns_third (third intersection): divisor identity becomes
  ...
  | WeierstrassCurve.Affine.Point.some (x := x) (y := y) hns =>
    ...
    · -- Use h_eqP to swap (x, y) ↔ P inside hns.
      ...
      have hns_P : E.toW.toAffine.Nonsingular P.1 P.2 := by
        rw [hP_eq]; exact hns
      have h_eq : (WeierstrassCurve.Affine.Point.some _ _ hns : ECPoint E)
                = WeierstrassCurve.Affine.Point.some _ _ hns_P := by
        rw [show (WeierstrassCurve.Affine.Point.some _ _ hns : ECPoint E)
              = ECPoint.affine E x y from (ECPoint.affine_of_nonsingular E hns).symm]
        rw [show (WeierstrassCurve.Affine.Point.some _ _ hns_P : ECPoint E)
    ...
    · by_cases h_eqQ : (x, y) = Q
      · have hQ_eq : Q = (x, y) := h_eqQ.symm
        have hns_Q : E.toW.toAffine.Nonsingular Q.1 Q.2 := by
          rw [hQ_eq]; exact hns
        have h_eq : (WeierstrassCurve.Affine.Point.some _ _ hns : ECPoint E)
                  = WeierstrassCurve.Affine.Point.some _ _ hns_Q := by
          rw [show (WeierstrassCurve.Affine.Point.some _ _ hns : ECPoint E)
                = ECPoint.affine E x y from (ECPoint.affine_of_nonsingular E hns).symm]
          rw [show (WeierstrassCurve.Affine.Point.some _ _ hns_Q : ECPoint E)
      ...
      · by_cases h_eqA₂ : (x, y) = (slopeOf P.1 P.2 Q.1 Q.2 ^ 2 - P.1 - Q.1,
        ...
        · have hThird_eq : (slopeOf P.1 P.2 Q.1 Q.2 ^ 2 - P.1 - Q.1,
          ...
          have h_eq : (WeierstrassCurve.Affine.Point.some _ _ hns : ECPoint E)
                    = WeierstrassCurve.Affine.Point.some _ _ hns_third := by
            rw [show (WeierstrassCurve.Affine.Point.some _ _ hns : ECPoint E)
                  = ECPoint.affine E x y from (ECPoint.affine_of_nonsingular E hns).symm]
            rw [show (WeierstrassCurve.Affine.Point.some _ _ hns_third : ECPoint E)
  ...
  | WeierstrassCurve.Affine.Point.some (x := x) (y := y) hns =>
    ...
    · -- x = P.1: derive y = P.2 or y = -P.2.
      ...
      · -- y = -P.2.
        ...
        have h_eq : (WeierstrassCurve.Affine.Point.some _ _ hns : ECPoint E)
                  = WeierstrassCurve.Affine.Point.some _ _ hns_negP := by
          rw [show (WeierstrassCurve.Affine.Point.some _ _ hns : ECPoint E)
                = ECPoint.affine E x y from (ECPoint.affine_of_nonsingular E hns).symm]
          rw [show (WeierstrassCurve.Affine.Point.some _ _ hns_negP : ECPoint E)
...
theorem accInv_poly_vanishes_at_neg_point
  ...
  have h_div_at := h_div (WeierstrassCurve.Affine.Point.some _ _ hns_neg)
  -- divisorOfD a.poly at .some _ _ hns_neg = ordAt a.poly (a.point.1, -a.point.2) (cast).
  rw [show (WeierstrassCurve.Affine.Point.some _ _ hns_neg : ECPoint E)
  ...
  have h_residue_at_neg : residueDivisor E (ECPoint.affineOfMem E h_pt_mem)
    ...
    · -- ECPoint.affine E a.point.1 (-a.point.2) = -ECPoint.affineOfMem E h_pt_mem.
      ...
      -- -.some _ _ hns_pt = .some (negated witness) with x same, y negated.
      rw [show -((WeierstrassCurve.Affine.Point.some _ _ hns_pt) : ECPoint E)
...
theorem accInv_combine_higher_distinct_divisor_at_third_AccInv_form
  ...
  have h_formal_zero : formalDivisorOfList E (xs ++ ys)
    ...
    rw [formalDivisorOfList_append, show
      formalDivisorOfList E xs (WeierstrassCurve.Affine.Point.some _ _ hns
                                : ECPoint E)
      = ((xs.filter (fun P => P = (Q₀x, Q₀y))).length : ℤ) from rfl,
      show formalDivisorOfList E ys (WeierstrassCurve.Affine.Point.some _ _ hns
                                     : ECPoint E)
...
theorem accInv_combine_higher_distinct_divisor_at_combine_lift_AccInv_form
  ...
  have h_formal_zero : formalDivisorOfList E (xs ++ ys)
    ...
    rw [ECPoint.affine_of_nonsingular E hns_neg]
    rw [formalDivisorOfList_append, show
      formalDivisorOfList E xs (WeierstrassCurve.Affine.Point.some _ _ hns_neg
                                : ECPoint E)
      = ((xs.filter (fun P => P = (Q₀x, -Q₀y))).length : ℤ) from rfl,
      show formalDivisorOfList E ys (WeierstrassCurve.Affine.Point.some _ _ hns_neg
                                     : ECPoint E)
      = ((ys.filter (fun P => P = (Q₀x, -Q₀y))).length : ℤ) from rfl]
    have h1 : xs.filter (fun P => P = (Q₀x, -Q₀y)) = [] := by
      apply List.filter_eq_nil_iff.mpr
  ...
  · -- R ≠ -combine.lift.
    ...
    · rw [ECPoint.affine_of_nonsingular E hns_neg] at h_eq
      ...
      -- h_eq : .some _ _ hns_neg = .some _ _ hQ₀ns, forcing -Q₀y = Q₀y.
...
/-! ### Chord nonvanishes at -a.lift, divisor = 0

Chord at sheet `(a.x, -a.y)` is `-2 a.y ≠ 0` (when a.y ≠ 0), so chord
doesn't vanish there. Hence `divisorOfD chord (-a.lift) = 0`. -/

theorem chordCoordRingElt_divisor_at_neg_a_lift_eq_zero
    (a b : EagenAccum E)
    (ha : a.point ∈ E.points)
    (h_xx : a.point.1 ≠ b.point.1)
    (hY_a : a.point.2 ≠ 0) :
    divisorOfD E (chordCoordRingElt E a.point b.point)
      (ECPoint.affine E a.point.1 (-a.point.2)) = 0 := by
  classical
  have h_chord_at_neg_a :
      (chordCoordRingElt E a.point b.point).eval a.point.1 (-a.point.2) ≠ 0 := by
    rw [chordCoordRingElt_eval_at_neg_a E a b h_xx]
    show (-2 : ZMod E.q) * a.point.2 ≠ 0
    have h2 : (2 : ZMod E.q) ≠ 0 := ZMod_two_ne_zero_of_E E
    exact mul_ne_zero (neg_ne_zero.mpr h2) hY_a
  ...
  have h_neg_a_pt : (a.point.1, -a.point.2) ∈ E.points := by
    apply E.hComplete
    have hC := E.hOnCurve _ ha
    show (-a.point.2) ^ 2 = a.point.1 ^ 3 + E.curveA * a.point.1 + E.curveB
    rw [neg_pow_two]; exact hC
  have hns : E.toW.toAffine.Nonsingular a.point.1 (-a.point.2) :=
    E.equation_iff_nonsingular.mp ((E.equation_iff _ _).mpr (E.hOnCurve _ h_neg_a_pt))
  rw [ECPoint.affine_of_nonsingular E hns]
  show (ordAt E (chordCoordRingElt E a.point b.point) (a.point.1, -a.point.2) : ℤ) = 0
  rw [ordAt_eq_zero_of_eval_ne_zero E _ h_neg_a_pt h_chord_at_neg_a]
  rfl

/-! ### Combine step at -a.lift AccInv-form divisor identity -/

theorem accInv_combine_higher_distinct_divisor_at_neg_a_lift_AccInv_form
    {xs ys : List (ZMod E.q × ZMod E.q)} {a b : EagenAccum E}
    (h_acc_a : AccInv E xs a) (h_acc_b : AccInv E ys b)
    (h_xx : a.point.1 ≠ b.point.1)
    (hY_a : a.point.2 ≠ 0) (hY_b : b.point.2 ≠ 0)
    ...
    (h_b_at_a : b.poly.eval a.point.1 a.point.2 ≠ 0)
    (h_b_at_neg_a : b.poly.eval a.point.1 (-a.point.2) ≠ 0)
    (h_neg_a_lift_ne_neg_b_lift :
        (-(ECPoint.affineOfMem E h_acc_a.1) : ECPoint E)
          ≠ -(ECPoint.affineOfMem E h_acc_b.1 : ECPoint E))
    (h_neg_a_lift_ne_neg_combine_lift :
        (-(ECPoint.affineOfMem E h_acc_a.1) : ECPoint E)
          ≠ -(ECPoint.affineOfMem E
              (combine_higher_distinct_running_sum E a b
                  h_acc_a.1 h_acc_b.1 h_xx).choose : ECPoint E)) :
  ...
  obtain ⟨h_combine_pt, _⟩ := combine_higher_distinct_running_sum
    E a b h_a_pt h_b_pt h_xx
  refine ⟨h_combine_pt, ?_⟩
  -- (a.x, -a.y) ∈ E.points; -a.lift = ECPoint.affine a.x (-a.y).
  have h_neg_a_pt : (a.point.1, -a.point.2) ∈ E.points := by
    apply E.hComplete
    have hC := E.hOnCurve _ h_a_pt
    show (-a.point.2) ^ 2 = a.point.1 ^ 3 + E.curveA * a.point.1 + E.curveB
  ...
  have hns_neg_a : E.toW.toAffine.Nonsingular a.point.1 (-a.point.2) :=
    E.equation_iff_nonsingular.mp ((E.equation_iff _ _).mpr (E.hOnCurve _ h_neg_a_pt))
  -- ECPoint.affine a.x (-a.y) = -a.lift via affine_neg.
  have h_R_eq_neg_a : (ECPoint.affine E a.point.1 (-a.point.2) : ECPoint E)
      = -(ECPoint.affineOfMem E h_a_pt : ECPoint E) := by
    rw [← ECPoint.affine_eq_affineOfMem E h_a_pt]
    exact (ECPoint.affine_neg E a.point.1 a.point.2).symm
  -- -a.lift ≠ 0.
  have h_neg_a_ne_zero : (-(ECPoint.affineOfMem E h_a_pt) : ECPoint E) ≠ 0 := by
    intro h_eq
    rw [← h_R_eq_neg_a] at h_eq
    rw [ECPoint.affine_of_nonsingular E hns_neg_a] at h_eq
    cases h_eq
  -- Apply universal divLin double-subtract.
  rw [combine_higher_distinct_divisor_via_prod_minus_two_verts (E := E)
        h_acc_a h_acc_b h_xx hY_a hY_b h_a_poly_NZ h_b_poly_NZ
        (ECPoint.affine E a.point.1 (-a.point.2))]
  -- Use prod_split (b.poly nonvanish on a fiber: same as at a.lift case).
  -- The fiber {R, -R} = {(a.x, -a.y), (a.x, a.y)}; reuse h_b_at_neg_a, h_b_at_a.
  rw [combine_higher_distinct_prod_divisor_split_when_b_nonvanish_fiber (E := E)
        h_a_pt h_b_pt h_xx h_a_poly_NZ h_b_poly_NZ
        h_Q₀x_ne_a.symm h_Q₀x_ne_b.symm
        h_neg_a_pt h_b_at_neg_a (by show b.poly.eval a.point.1 (- -a.point.2) ≠ 0;
                                     rw [neg_neg]; exact h_b_at_a)]
  -- div(chord)(-a.lift) = 0.
  rw [chordCoordRingElt_divisor_at_neg_a_lift_eq_zero E a b h_a_pt h_xx hY_a]
  -- div(vert(a.x))(-a.lift) = 1.
  have hY_neg : -a.point.2 ≠ 0 := neg_ne_zero.mpr hY_a
  rw [divisorOfD_vertical_at_x₀_nonTwoTorsion_affine E a.point.1 (-a.point.2)
        h_neg_a_pt hY_neg]
  -- div(vert(b.x))(-a.lift) = 0.
  rw [divisorOfD_vertical_at_off_x₀_affine E b.point.1 h_neg_a_pt h_xx]
  -- AccInv divisor identities at -a.lift = -a.affineOfMem.
  obtain ⟨_, _, h_div_a⟩ := h_acc_a
  obtain ⟨_, _, h_div_b⟩ := h_acc_b
  rw [h_R_eq_neg_a]
  rw [h_div_a (-(ECPoint.affineOfMem E h_a_pt) : ECPoint E)]
  rw [h_div_b (-(ECPoint.affineOfMem E h_a_pt) : ECPoint E)]
  -- residueDivisor a.lift (-a.lift) = 1 (since -a.lift = -a.lift, and -a.lift ≠ 0).
  have h_residue_a_at_neg_a : residueDivisor E (ECPoint.affineOfMem E h_a_pt)
        (-(ECPoint.affineOfMem E h_a_pt) : ECPoint E) = 1 := by
    unfold residueDivisor
    rw [if_pos rfl, if_neg h_neg_a_ne_zero]
    ring
  rw [h_residue_a_at_neg_a]
  -- residueDivisor b.lift (-a.lift) = 0 (-a.lift ≠ -b.lift hypothesis, ≠ 0).
  have h_residue_b_at_neg_a : residueDivisor E (ECPoint.affineOfMem E h_b_pt)
        (-(ECPoint.affineOfMem E h_a_pt) : ECPoint E) = 0 :=
    residueDivisor_at_other E _ _ h_neg_a_lift_ne_neg_b_lift h_neg_a_ne_zero
  rw [h_residue_b_at_neg_a]
  -- residueDivisor combine.lift (-a.lift) = 0.
  have h_residue_combine_at_neg_a : residueDivisor E
        (ECPoint.affineOfMem E h_combine_pt)
        (-(ECPoint.affineOfMem E h_a_pt) : ECPoint E) = 0 :=
    residueDivisor_at_other E _ _
      (by convert h_neg_a_lift_ne_neg_combine_lift using 2)
      h_neg_a_ne_zero
  rw [h_residue_combine_at_neg_a, add_zero]
  -- formalDivisorOfList split.
  rw [formalDivisorOfList_append]
  ring
...
/-! ### Chord b-side helpers -/

theorem chordCoordRingElt_eval_at_neg_b
    (a b : EagenAccum E)
    (h_xx : a.point.1 ≠ b.point.1) :
    (chordCoordRingElt E a.point b.point).eval b.point.1 (-b.point.2)
      = -2 * b.point.2 := by
  classical
  rw [chordCoordRingElt_eval_eq_lineThrough_chord E h_xx (b.point.1, -b.point.2)]
  unfold Line.eval lineThrough
  show -b.point.2 - slopeOf a.point.1 a.point.2 b.point.1 b.point.2 * b.point.1
        - (a.point.2 - slopeOf a.point.1 a.point.2 b.point.1 b.point.2 * a.point.1)
      = -2 * b.point.2
  -- Expand slopeOf using b.point.2 = a.point.2 + slope * (b.point.1 - a.point.1):
  -- specifically slope * (b.point.1 - a.point.1) = b.point.2 - a.point.2 by construction.
  unfold slopeOf
  field_simp
  have hxx_ne : b.point.1 - a.point.1 ≠ 0 := sub_ne_zero.mpr (Ne.symm h_xx)
  field_simp
  ring

theorem chordCoordRingElt_divisor_at_b_lift_eq_one
    (a b : EagenAccum E)
    (ha : a.point ∈ E.points) (hb : b.point ∈ E.points)
    (h_xx : a.point.1 ≠ b.point.1)
    (hY_b : b.point.2 ≠ 0)
    ...
    (h_Q₀x_ne_b : (slopeOf a.point.1 a.point.2 b.point.1 b.point.2 ^ 2
                    - a.point.1 - b.point.1) ≠ b.point.1) :
    divisorOfD E (chordCoordRingElt E a.point b.point)
      (ECPoint.affine E b.point.1 b.point.2) = 1 := by
  classical
  have h_chord_at_b : (chordCoordRingElt E a.point b.point).eval b.point.1 b.point.2 = 0 :=
    chordCoordRingElt_eval_right E a.point b.point
  ...
  have h_chord_NZ : ¬ ((chordCoordRingElt E a.point b.point).a = 0
    ...
    chordCoordRingElt_ne_zero E a.point b.point
  have h_ordAt :=
    ordAt_lone_sheet_eq_rootMult_normPoly (E := E) h_chord_NZ hb hY_b
      h_chord_at_b h_chord_at_neg_b
  have h_rootMult_b : Polynomial.rootMultiplicity b.point.1
      (normPoly E (chordCoordRingElt E a.point b.point)) = 1 := by
    have h_le := chordCoordRingElt_normPoly_rootMult_le_one_at_distinct_chord E
      a.point b.point ha hb h_xx h_Q₀x_ne_a.symm h_Q₀x_ne_b.symm b.point.1
    have h_normPoly_zero :
        (normPoly E (chordCoordRingElt E a.point b.point)).eval b.point.1 = 0 := by
      rw [normPoly_eval_eq_D_mul_D_neg E (chordCoordRingElt E a.point b.point) hb]
      rw [h_chord_at_b, zero_mul]
    have h_normPoly_NZ : normPoly E (chordCoordRingElt E a.point b.point) ≠ 0 :=
      normPoly_ne_zero E _ h_chord_NZ
    have h_pos : 0 < Polynomial.rootMultiplicity b.point.1
        (normPoly E (chordCoordRingElt E a.point b.point)) :=
      (Polynomial.rootMultiplicity_pos h_normPoly_NZ).mpr h_normPoly_zero
    omega
  have hns : E.toW.toAffine.Nonsingular b.point.1 b.point.2 :=
    E.equation_iff_nonsingular.mp ((E.equation_iff _ _).mpr (E.hOnCurve _ hb))
  rw [ECPoint.affine_of_nonsingular E hns]
  show (ordAt E (chordCoordRingElt E a.point b.point) (b.point.1, b.point.2) : ℤ) = 1
  rw [h_ordAt, h_rootMult_b]
  rfl

/-! ### Combine step at-b.lift AccInv-form divisor identity -/

theorem accInv_combine_higher_distinct_divisor_at_b_lift_AccInv_form
    {xs ys : List (ZMod E.q × ZMod E.q)} {a b : EagenAccum E}
    (h_acc_a : AccInv E xs a) (h_acc_b : AccInv E ys b)
    (h_xx : a.point.1 ≠ b.point.1)
    (hY_a : a.point.2 ≠ 0) (hY_b : b.point.2 ≠ 0)
    ...
    (h_b_lift_ne_neg_a_lift :
        ECPoint.affineOfMem E h_acc_b.1
          ≠ -(ECPoint.affineOfMem E h_acc_a.1 : ECPoint E))
    (h_b_lift_ne_neg_combine_lift :
        ECPoint.affineOfMem E h_acc_b.1
          ≠ -(ECPoint.affineOfMem E
              (combine_higher_distinct_running_sum E a b
                  h_acc_a.1 h_acc_b.1 h_xx).choose : ECPoint E)) :
  ...
  have h_b_lift_ne_zero :
      (ECPoint.affineOfMem E h_b_pt : ECPoint E) ≠ 0 := by
    intro h_eq
    unfold ECPoint.affineOfMem ECPoint.affineOfEqn at h_eq
    cases h_eq
  have hns_b : E.toW.toAffine.Nonsingular b.point.1 b.point.2 :=
    E.equation_iff_nonsingular.mp ((E.equation_iff _ _).mpr (E.hOnCurve _ h_b_pt))
  have h_R_eq : (ECPoint.affine E b.point.1 b.point.2 : ECPoint E)
      = ECPoint.affineOfMem E h_b_pt := ECPoint.affine_eq_affineOfMem E h_b_pt
  -- Apply universal divLin double-subtract.
  rw [combine_higher_distinct_divisor_via_prod_minus_two_verts (E := E)
        h_acc_a h_acc_b h_xx hY_a hY_b h_a_poly_NZ h_b_poly_NZ
        (ECPoint.affine E b.point.1 b.point.2)]
  -- Use a-side prod_split (a.poly nonvanish on b fiber).
  rw [combine_higher_distinct_prod_divisor_split_when_a_nonvanish_fiber (E := E)
        h_a_pt h_b_pt h_xx h_a_poly_NZ h_b_poly_NZ
        h_Q₀x_ne_a.symm h_Q₀x_ne_b.symm
        h_b_pt h_a_at_b h_a_at_neg_b]
  -- div(chord)(b.lift) = 1.
  rw [chordCoordRingElt_divisor_at_b_lift_eq_one E a b h_a_pt h_b_pt h_xx hY_b
        h_Q₀x_ne_a h_Q₀x_ne_b]
  -- div(vert(a.x))(b.lift) = 0 (a.x ≠ b.x).
  rw [divisorOfD_vertical_at_off_x₀_affine E a.point.1 h_b_pt (Ne.symm h_xx)]
  -- div(vert(b.x))(b.lift) = 1.
  rw [divisorOfD_vertical_at_x₀_nonTwoTorsion_affine E b.point.1 b.point.2
        h_b_pt hY_b]
  -- AccInv divisor identities at b.lift.
  obtain ⟨_, _, h_div_a⟩ := h_acc_a
  obtain ⟨_, _, h_div_b⟩ := h_acc_b
  rw [h_R_eq]
  rw [h_div_a (ECPoint.affineOfMem E h_b_pt)]
  rw [h_div_b (ECPoint.affineOfMem E h_b_pt)]
  -- residueDivisor a.lift (b.lift) = 0 (b.lift ≠ -a.lift hypothesis, b.lift ≠ 0).
  have h_residue_a_at_b : residueDivisor E (ECPoint.affineOfMem E h_a_pt)
        (ECPoint.affineOfMem E h_b_pt) = 0 :=
    residueDivisor_at_other E _ _ h_b_lift_ne_neg_a_lift h_b_lift_ne_zero
  rw [h_residue_a_at_b]
  -- residueDivisor b.lift (b.lift) = 0 (b.lift ≠ -b.lift since b.y ≠ 0; b.lift ≠ 0).
  have h_b_ne_neg_b : (ECPoint.affineOfMem E h_b_pt : ECPoint E)
    ...
    intro h_eq
    rw [ECPoint.affine_of_nonsingular E hns_b] at h_eq
    ...
    have h2 : (2 : ZMod E.q) ≠ 0 := ZMod_two_ne_zero_of_E E
    apply hY_b
    have : (2 : ZMod E.q) * b.point.2 = 0 := by linear_combination h_y_eq
    rcases mul_eq_zero.mp this with h | h
    · exact absurd h h2
    · exact h
  have h_residue_b_at_b : residueDivisor E (ECPoint.affineOfMem E h_b_pt)
        (ECPoint.affineOfMem E h_b_pt) = 0 :=
    residueDivisor_at_other E _ _ h_b_ne_neg_b h_b_lift_ne_zero
  rw [h_residue_b_at_b]
  -- residueDivisor combine.lift (b.lift) = 0.
  have h_residue_combine_at_b : residueDivisor E
        (ECPoint.affineOfMem E h_combine_pt) (ECPoint.affineOfMem E h_b_pt) = 0 :=
    residueDivisor_at_other E _ _
      (by convert h_b_lift_ne_neg_combine_lift using 2)
      h_b_lift_ne_zero
  rw [h_residue_combine_at_b, add_zero]
  -- formalDivisorOfList split.
  rw [formalDivisorOfList_append]
  ring
...
/-! ### Combine step divisor identity at general affine R off chord support

For R = ECPoint.affine x y with x ≠ a.x, b.x and chord, b.poly both
nonvanishing on R's fiber, plus R off the residue points (-a.lift,
-b.lift, -combine.lift): the AccInv divisor identity holds. R may
lie in xs or ys formalDivisor support without issue. -/

theorem accInv_combine_higher_distinct_divisor_at_general_off_chord_AccInv_form
    {xs ys : List (ZMod E.q × ZMod E.q)} {a b : EagenAccum E}
    (h_acc_a : AccInv E xs a) (h_acc_b : AccInv E ys b)
    (h_xx : a.point.1 ≠ b.point.1)
    (hY_a : a.point.2 ≠ 0) (hY_b : b.point.2 ≠ 0)
  ...
  rw [combine_higher_distinct_divisor_via_prod_minus_two_verts (E := E)
        h_acc_a h_acc_b h_xx hY_a hY_b h_a_poly_NZ h_b_poly_NZ
        (ECPoint.affine E x y)]
  -- b-side prod_split (b.poly nonvanish).
  rw [combine_higher_distinct_prod_divisor_split_when_b_nonvanish_fiber (E := E)
        h_a_pt h_b_pt h_xx h_a_poly_NZ h_b_poly_NZ
        h_Q₀x_ne_a.symm h_Q₀x_ne_b.symm
        hP h_b_at_pos h_b_at_neg]
  -- div(chord)(R) = 0 (chord nonvanish at R).
  have h_div_chord_zero :
      divisorOfD E (chordCoordRingElt E a.point b.point)
        (ECPoint.affine E x y) = 0 := by
    rw [ECPoint.affine_of_nonsingular E hns]
    show (ordAt E (chordCoordRingElt E a.point b.point) (x, y) : ℤ) = 0
    rw [ordAt_eq_zero_of_eval_ne_zero E _ hP h_chord_pos]
    rfl
  rw [h_div_chord_zero]
  -- div(vert(a.x))(R) = 0 (x ≠ a.x).
  rw [divisorOfD_vertical_at_off_x₀_affine E a.point.1 hP h_x_ne_a]
  -- div(vert(b.x))(R) = 0 (x ≠ b.x).
  rw [divisorOfD_vertical_at_off_x₀_affine E b.point.1 hP h_x_ne_b]
  -- AccInv divisor identities for a.poly, b.poly at R.
  obtain ⟨_, _, h_div_a⟩ := h_acc_a
  obtain ⟨_, _, h_div_b⟩ := h_acc_b
  rw [h_div_a (ECPoint.affine E x y)]
  rw [h_div_b (ECPoint.affine E x y)]
  -- residueDivisor a.lift R = 0 (R ≠ -a.lift hypothesis).
  ...
  rw [residueDivisor_at_other E _ _
      (by convert h_R_ne_neg_combine_lift using 2)
      h_R_ne_zero]
  rw [add_zero]
  rw [formalDivisorOfList_append]
  ring
...
theorem accInv_combine_higher_distinct_step_at_third
    {xs ys : List (ZMod E.q × ZMod E.q)} {a b : EagenAccum E}
    (h_acc_a : AccInv E xs a) (h_acc_b : AccInv E ys b)
    (h_xx : a.point.1 ≠ b.point.1)
    (hY_a : a.point.2 ≠ 0) (hY_b : b.point.2 ≠ 0)
    ...
    ∃ h_combine_pt : combine.point ∈ E.points,
      divisorOfD E combine.poly (ECPoint.affine E Q₀x Q₀y)
        = formalDivisorOfList E (xs ++ ys) (ECPoint.affine E Q₀x Q₀y)
          + residueDivisor E (ECPoint.affineOfMem E h_combine_pt)
              (ECPoint.affine E Q₀x Q₀y) :=
  accInv_combine_higher_distinct_divisor_at_third_AccInv_form (E := E)
    h_acc_a h_acc_b h_xx hY_a hY_b h_a_poly_NZ h_b_poly_NZ
    hStrong.hQ₀x_ne_a hStrong.hQ₀x_ne_b hStrong.hQ₀y_ne_zero
    hStrong.ha_at_Q₀ hStrong.ha_at_neg_Q₀ hStrong.hb_at_Q₀ hStrong.hb_at_neg_Q₀
    hStrong.hQ₀_not_in_xs hStrong.hQ₀_not_in_ys

theorem accInv_combine_higher_distinct_step_at_combine_lift
    {xs ys : List (ZMod E.q × ZMod E.q)} {a b : EagenAccum E}
    (h_acc_a : AccInv E xs a) (h_acc_b : AccInv E ys b)
    (h_xx : a.point.1 ≠ b.point.1)
    (hY_a : a.point.2 ≠ 0) (hY_b : b.point.2 ≠ 0)
    ...
    ∃ h_combine_pt : combine.point ∈ E.points,
      divisorOfD E combine.poly (ECPoint.affine E Q₀x (-Q₀y))
        = formalDivisorOfList E (xs ++ ys) (ECPoint.affine E Q₀x (-Q₀y))
          + residueDivisor E (ECPoint.affineOfMem E h_combine_pt)
              (ECPoint.affine E Q₀x (-Q₀y)) :=
  accInv_combine_higher_distinct_divisor_at_combine_lift_AccInv_form (E := E)
    h_acc_a h_acc_b h_xx hY_a hY_b h_a_poly_NZ h_b_poly_NZ
    hStrong.hQ₀x_ne_a hStrong.hQ₀x_ne_b hStrong.hQ₀y_ne_zero
...
theorem accInv_combine_higher_distinct_step_divisor_identity
  ...
  | some x y hns =>
      ...
      -- ECPoint.affine E x y = .some _ _ hns.
...
theorem divisorOfD_vertical_at_any_R
  ...
  | WeierstrassCurve.Affine.Point.some _ _ hns =>
    rename_i x' y'
    -- R = .some _ _ hns at (x', y'). (x', y') ∈ E.points.
    have hP' : (x', y') ∈ E.points :=
      E.hComplete x' y' ((E.equation_iff x' y').mp
        ((E.equation_iff_nonsingular).mpr hns))
    have h_R_ne_zero : (WeierstrassCurve.Affine.Point.some _ _ hns : ECPoint E) ≠ 0 := by
      intro h; cases h
    ...
    · -- x' = x: y-dichotomy gives y' = y or y' = -y.
      ...
      · -- y' = y: R = ECPoint.affine x y.
        have h_R_eq : (WeierstrassCurve.Affine.Point.some _ _ hns : ECPoint E)
            = ECPoint.affine E x y := by
          subst h_x_eq h_y
    ...
    · -- x' ≠ x: vert nonvanish at R.
      have h_R_ne_pos : (WeierstrassCurve.Affine.Point.some _ _ hns : ECPoint E)
          ≠ ECPoint.affine E x y := by
        rw [ECPoint.affine_of_nonsingular E
            (E.equation_iff_nonsingular.mp ((E.equation_iff _ _).mpr (E.hOnCurve _ hP)))]
        intro h_eq; injection h_eq with hx _; exact h_x_eq hx
      have h_R_ne_neg : (WeierstrassCurve.Affine.Point.some _ _ hns : ECPoint E)
          ≠ ECPoint.affine E x (-y) := by
        rw [ECPoint.affine_of_nonsingular E
            (E.equation_iff_nonsingular.mp ((E.equation_iff _ _).mpr (E.hOnCurve _ h_neg_pt)))]
        intro h_eq; injection h_eq with hx _; exact h_x_eq hx
...
/-! ### AccInv ⟹ normPoly rootMult ≤ 1 at the accumulator's own x-coord

Under AccInv, plus xs disjoint from {a.point, -a.point}, the
normPoly's rootMult at a.point.1 is exactly 1 (residue at -a.lift
contributes 1, formal counts contribute 0). -/

theorem accInv_normPoly_rootMult_at_a_x_eq_one
    {xs : List (ZMod E.q × ZMod E.q)} {a : EagenAccum E}
    (h_acc : AccInv E xs a)
    (hY_a : a.point.2 ≠ 0)
    (h_a_poly_NZ : ¬ (a.poly.a = 0 ∧ a.poly.b = 0))
    ...
    Polynomial.rootMultiplicity a.point.1 (normPoly E a.poly) = 1 := by
  classical
  -- Compute via norm-sum identity:
  --   rootMult a.x (normPoly a.poly) = ordAt(a.poly)(a.lift) + ordAt(a.poly)(-a.lift)
  -- AccInv gives both ords:
  --   ordAt(a.poly)(a.lift) = formal_xs(a.lift) + 0 = 0 (xs has no a.point)
  --   ordAt(a.poly)(-a.lift) = formal_xs(-a.lift) + 1 = 0 + 1 = 1.
  -- Sum = 1.
  have h_a_pt : a.point ∈ E.points := h_acc.1
  have h_neg_a_pt : (a.point.1, -a.point.2) ∈ E.points := by
    apply E.hComplete
    have hC := E.hOnCurve _ h_a_pt
    show (-a.point.2) ^ 2 = a.point.1 ^ 3 + E.curveA * a.point.1 + E.curveB
    rw [neg_pow_two]; exact hC
  -- Switch to a.point as the "P" in pair_eq_rootMult.
  have h_pair := ordAt_nonTwoTorsion_pair_eq_rootMult E a.poly h_a_poly_NZ
                  (P := a.point) h_a_pt hY_a
  -- a.point.1 = a.point.fst.
  show Polynomial.rootMultiplicity a.point.1 (normPoly E a.poly) = 1
  rw [← h_pair]
  -- Convert ordAt_nonTwoTorsion to ordAt using dispatch.
  have h_eq1 : ordAt_nonTwoTorsion E a.poly a.point = ordAt E a.poly a.point := by
    rw [ordAt_eq_dispatch E a.poly h_a_pt h_a_poly_NZ, if_neg hY_a]
  have h_eq2 : ordAt_nonTwoTorsion E a.poly (a.point.1, -a.point.2)
              = ordAt E a.poly (a.point.1, -a.point.2) := by
    have hY_neg : (-a.point.2 : ZMod E.q) ≠ 0 := neg_ne_zero.mpr hY_a
    rw [ordAt_eq_dispatch E a.poly h_neg_a_pt h_a_poly_NZ, if_neg hY_neg]
  rw [h_eq1, h_eq2]
  -- Now compute via AccInv.
  have h_div_a := h_acc.2.2 (ECPoint.affineOfMem E h_a_pt)
  have h_div_neg_a := h_acc.2.2 (ECPoint.affineOfMem E h_neg_a_pt)
  -- divisorOfD a.poly (affineOfMem h_a_pt) = (ordAt a.poly a.point : ℤ).
  rw [show divisorOfD E a.poly (ECPoint.affineOfMem E h_a_pt)
        = (ordAt E a.poly a.point : ℤ) from by
      rw [← ECPoint.affine_eq_affineOfMem E h_a_pt]
      rw [ECPoint.affine_of_nonsingular E
          (E.equation_iff_nonsingular.mp ((E.equation_iff _ _).mpr (E.hOnCurve _ h_a_pt)))]
      rfl] at h_div_a
  -- Similarly for -a.lift.
  rw [show divisorOfD E a.poly (ECPoint.affineOfMem E h_neg_a_pt)
        = (ordAt E a.poly (a.point.1, -a.point.2) : ℤ) from by
      rw [← ECPoint.affine_eq_affineOfMem E h_neg_a_pt]
      rw [ECPoint.affine_of_nonsingular E
          (E.equation_iff_nonsingular.mp
            ((E.equation_iff _ _).mpr (E.hOnCurve _ h_neg_a_pt)))]
      rfl] at h_div_neg_a
  -- formalDivisorOfList xs at a.lift = 0 (xs has no a.point).
  have h_formal_a : formalDivisorOfList E xs (ECPoint.affineOfMem E h_a_pt) = 0 := by
    rw [← ECPoint.affine_eq_affineOfMem E h_a_pt]
    rw [ECPoint.affine_of_nonsingular E
        (E.equation_iff_nonsingular.mp ((E.equation_iff _ _).mpr (E.hOnCurve _ h_a_pt)))]
    show ((xs.filter (fun P => P = (a.point.1, a.point.2))).length : ℤ) = 0
    have h_filt : xs.filter (fun P => P = (a.point.1, a.point.2)) = [] := by
      apply List.filter_eq_nil_iff.mpr
      intro P hP_in heq
      apply h_a_pt_not_in_xs
      have : P = a.point := decide_eq_true_eq.mp heq
      rwa [this] at hP_in
    rw [h_filt]; simp
  have h_formal_neg_a : formalDivisorOfList E xs (ECPoint.affineOfMem E h_neg_a_pt) = 0 := by
    rw [← ECPoint.affine_eq_affineOfMem E h_neg_a_pt]
    rw [ECPoint.affine_of_nonsingular E
        (E.equation_iff_nonsingular.mp
          ((E.equation_iff _ _).mpr (E.hOnCurve _ h_neg_a_pt)))]
    ...
    have h_filt : xs.filter (fun P => P = (a.point.1, -a.point.2)) = [] := by
      apply List.filter_eq_nil_iff.mpr
      intro P hP_in heq
      apply h_neg_a_pt_not_in_xs
      have : P = (a.point.1, -a.point.2) := decide_eq_true_eq.mp heq
      rwa [this] at hP_in
    rw [h_filt]; simp
  -- residueDivisor a.lift (a.lift) = 0 since a.lift ≠ -a.lift (hY_a).
  have h_a_lift_ne_zero :
      (ECPoint.affineOfMem E h_a_pt : ECPoint E) ≠ 0 := by
    intro h_eq
    unfold ECPoint.affineOfMem ECPoint.affineOfEqn at h_eq
  ...
  have h_residue_a_at_a : residueDivisor E (ECPoint.affineOfMem E h_a_pt)
        (ECPoint.affineOfMem E h_a_pt) = 0 :=
    residueDivisor_at_other E _ _ h_a_ne_neg_a h_a_lift_ne_zero
  -- residueDivisor a.lift (-a.lift) = 1.
  have h_neg_a_ne_zero :
      (ECPoint.affineOfMem E h_neg_a_pt : ECPoint E) ≠ 0 := by
    intro h_eq
    unfold ECPoint.affineOfMem ECPoint.affineOfEqn at h_eq
    cases h_eq
  have h_neg_a_eq : (ECPoint.affineOfMem E h_neg_a_pt : ECPoint E)
      = -(ECPoint.affineOfMem E h_a_pt : ECPoint E) := by
    rw [← ECPoint.affine_eq_affineOfMem E h_neg_a_pt]
    rw [← ECPoint.affine_eq_affineOfMem E h_a_pt]
    exact (ECPoint.affine_neg E a.point.1 a.point.2).symm
  have h_residue_a_at_neg_a : residueDivisor E (ECPoint.affineOfMem E h_a_pt)
        (ECPoint.affineOfMem E h_neg_a_pt) = 1 := by
    unfold residueDivisor
    rw [h_neg_a_eq]
    rw [if_pos rfl]
    rw [if_neg (by rw [← h_neg_a_eq]; exact h_neg_a_ne_zero)]
    ring
  rw [h_formal_a, h_residue_a_at_a, zero_add] at h_div_a
  rw [h_formal_neg_a, h_residue_a_at_neg_a, zero_add] at h_div_neg_a
  -- h_div_a : (ordAt a.poly a.point : ℤ) = 0.
  -- h_div_neg_a : (ordAt a.poly (a.point.1, -a.point.2) : ℤ) = 1.
  have h_ord_a : ordAt E a.poly a.point = 0 := by exact_mod_cast h_div_a
  have h_ord_neg_a : ordAt E a.poly (a.point.1, -a.point.2) = 1 := by
...
theorem terminalInv_combine_higher_vertical_at_neg_a_lift
    {xs ys : List (ZMod E.q × ZMod E.q)} {a b : EagenAccum E}
    (h_acc_a : AccInv E xs a) (h_acc_b : AccInv E ys b)
    (h_xx : a.point.1 = b.point.1) (h_yy : a.point.2 = -b.point.2)
    (hY_a : a.point.2 ≠ 0)
  ...
  rw [show mulCoordRingElt E a.poly b.poly = mulCoordRingElt E b.poly a.poly from
        mulCoordRingElt_comm E a.poly b.poly]
  rw [divisorOfD_mul_add_affine_when_normPoly_D2_le_one E h_b_poly_NZ h_a_poly_NZ
        (P := (a.point.1, -a.point.2)) h_neg_a_pt (by rw [h_root])]
  rw [h_acc_a.2.2 (ECPoint.affine E a.point.1 (-a.point.2))]
  rw [h_acc_b.2.2 (ECPoint.affine E a.point.1 (-a.point.2))]
  -- div(vert(a.x))(-a.lift) = 1.
  have hY_neg : -a.point.2 ≠ 0 := neg_ne_zero.mpr hY_a
  ...
  have h_neg_a_ne_zero : (-(ECPoint.affineOfMem E h_a_pt) : ECPoint E) ≠ 0 := by
    ...
    rw [← h_R_eq_neg_a] at h_eq
    rw [ECPoint.affine_of_nonsingular E
        (E.equation_iff_nonsingular.mp ((E.equation_iff _ _).mpr (E.hOnCurve _ h_neg_a_pt)))] at h_eq
    cases h_eq
  rw [h_R_eq_neg_a]
  have h_residue_a_at_neg_a : residueDivisor E (ECPoint.affineOfMem E h_a_pt)
        (-(ECPoint.affineOfMem E h_a_pt) : ECPoint E) = 1 := by
    unfold residueDivisor
    rw [if_pos rfl, if_neg h_neg_a_ne_zero]; ring
  rw [h_residue_a_at_neg_a]
  -- residueDivisor b.lift (-a.lift) = 0 (since b.lift = -a.lift, -b.lift = a.lift ≠ -a.lift).
  have h_b_lift_eq_neg_a : (ECPoint.affineOfMem E h_b_pt : ECPoint E)
      = -(ECPoint.affineOfMem E h_a_pt : ECPoint E) := by
    rw [← ECPoint.affine_eq_affineOfMem E h_a_pt]
    rw [← ECPoint.affine_eq_affineOfMem E h_b_pt]
    ...
    rw [show a.point.1 = b.point.1 from h_xx]
    rw [show -a.point.2 = b.point.2 from by rw [h_yy]; ring]
  -- -b.lift = -(-a.lift) = a.lift.
  -- We need residueDivisor b.lift (-a.lift) = (-a.lift = -b.lift ? 1 : 0) - (...).
  -- -b.lift = a.lift, and -a.lift ≠ a.lift (since hY_a, char ≠ 2).
  -- So -a.lift ≠ -b.lift, residueDivisor = 0.
  have h_neg_a_ne_neg_b : (-(ECPoint.affineOfMem E h_a_pt) : ECPoint E)
      ≠ -(ECPoint.affineOfMem E h_b_pt : ECPoint E) := by
    rw [h_b_lift_eq_neg_a, neg_neg]
    -- Need: -(a.lift) ≠ a.lift. Already established earlier.
    rw [← ECPoint.affine_eq_affineOfMem E h_a_pt]
    rw [show -(ECPoint.affine E a.point.1 a.point.2 : ECPoint E)
          = ECPoint.affine E a.point.1 (-a.point.2) from
        ECPoint.affine_neg E a.point.1 a.point.2]
    ...
    · exact absurd h h2
    · exact h
  rw [residueDivisor_at_other E _ _ h_neg_a_ne_neg_b h_neg_a_ne_zero]
  rw [formalDivisorOfList_append]
  ring
...
/-! ### eagenBuild on singleton input -/

theorem eagenBuild_singleton (P : ZMod E.q × ZMod E.q) :
    eagenBuild E [P]
      = ({ a := 1, b := 0 } : CoordRingElt E.q) := by
  show (match eagenBuild_iterate E ([P] : List (ZMod E.q × ZMod E.q)).length
              (eagenBuild_level0 E [P]) with
        | [] => ({ a := 1, b := 0 } : CoordRingElt E.q)
        | [single] => single.poly
        | _ => { a := 1, b := 0 }) = { a := 1, b := 0 }
  rw [eagenBuild_level0_singleton]
...
theorem divisorOfD_eagenBuild_nil_eq_formalDivisorOfList (R : ECPoint E) :
  ...
  | WeierstrassCurve.Affine.Point.some _ _ hns =>
    ...
    show divisorOfD E ({ a := 1, b := 0 } : CoordRingElt E.q)
            (WeierstrassCurve.Affine.Point.some _ _ hns) = 0
    show (ordAtPoint E ({ a := 1, b := 0 } : CoordRingElt E.q)
            (WeierstrassCurve.Affine.Point.some _ _ hns) : ℤ) = 0
    rw [show (WeierstrassCurve.Affine.Point.some _ _ hns : ECPoint E)
          = ECPoint.affine E x y from
        (ECPoint.affine_of_nonsingular E hns).symm]
