...
`(X - A₀.1)(X - A₁.1)(X - chordX₂ A₀ A₁)` over `(ZMod E.q)[X]`. -/
private theorem intersectionPoly_factorisation
  ...
  have hSum : A₀.1 + A₁.1 + x₂ = lam ^ 2 := by
    show A₀.1 + A₁.1 + (lam ^ 2 - A₀.1 - A₁.1) = _
    ring
  rw [hSum]
  rw [show A₀.1 * A₁.1 + A₀.1 * x₂ + A₁.1 * x₂ =
      E.curveA - 2 * lam * mu from he₂]
...
theorem chord_fiber_product_bar_factorisation
  ...
  -- `f` splits because `Fqbar E` is algebraically closed.
  ...
  -- Identify the root set of `f` with the image of the geometric
  -- support under `zLambdaBar lam`.
  have hRootSet :
  ...
  -- Regroup the product over `f.roots.toFinset` as a product over `gd.support`.
  have hReorg :
      (∏ z ∈ f.roots.toFinset, (X - C z) ^ (rootMultiplicity z f)) =
...
precisely of the lifts of `A₀`, `A₁`, and `(x₂, y₂)`. By `hDef`,
`D.eval` at each of these three rational points is nonzero, hence
`D.geomEval` at each lifted point is nonzero, so no lifted point lies in
`gd.support`. Therefore no `Q ∈ gd.support` is on the chord, which by
`lineEvalNumAtFullBar_eval_eq_zLambdaBar_diff` is equivalent to the
...
theorem geometric_chord_sum_eq_residue_sum
  ...
  have hCfpBarNe : eval (fqToBar E μ)
    ...
    rw [hCfpBarEval]
    exact mul_ne_zero hc_ne hPolNe
