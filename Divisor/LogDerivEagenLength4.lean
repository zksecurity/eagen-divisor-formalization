...
/-! ## HEADLINE: fully-discharged `weil_reciprocity_honest` for the simplest case

For the length-4 honest divisor structure with k=3 bases all at scalar 1
and input list `[-P_target, B 0, B 1, B 2]`, `logDerivCheckFn = 0` is
provable with NO hypotheses beyond on-curve + genericity + `¬bad`.
