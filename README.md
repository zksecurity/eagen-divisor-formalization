...
The following are axioms in the codebase but not on the current soundness/completeness surfaces:

- `Divisor.principal_divisor_iff` — Silverman AEC III Cor 3.5, full biconditional, $\mathbb{F}_q$-rational specialization (uses Remark 3.5.1 + Exercise II.2.13(b) for descent).
- `Divisor.hasse_weil` — Silverman AEC V Thm 1.1, p. 138, squared form $\bigl((\#E - q - 1)\bigr)^2 \le 4q$ (equivalent to the standard $|\#E - q - 1| \le 2\sqrt q$).
