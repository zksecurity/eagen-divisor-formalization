...
- The trace/log-derivative step is now a generic, chord-agnostic
  axiom `Polynomial.resultant_logDeriv_at_split_specialization_of_two_le_natDegree_pos_g`
  ...
  mathlib's `Polynomial.resultant_eq_prod_eval`) and the strengthened
  hypotheses `2 ≤ f.natDegree` and `0 < g.natDegree`. Three sub-cases
  are now theorems and excluded from the axiom: `f.natDegree = 0`,
  `f.natDegree = 1` (via `resultant_X_sub_C_left` plus the chain rule
  for `g.eval α`), and `g.natDegree = 0`. The
