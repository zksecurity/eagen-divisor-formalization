...
| `Polynomial.resultant_logDeriv_at_split_specialization` | **Generic narrow trace axiom** — Lang *Algebra* §VI.5 Thm 5.1 + §VIII.5 Thm 5.1 Case 1, specialised to the polynomial / resultant setting (the logarithmic derivative of `Res_X(f, g)` at a split inner-specialisation of `f`). Replaces the older project-shaped chord-specific axiom. See `axioms/resultant_logDeriv_at_split.md`. |
